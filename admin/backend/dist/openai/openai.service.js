...
- 必要に応じて「続きを生成しますか？」と確認`;
    ...
    detectOutputRequest(content) {
        ...
        return outputKeywords.some((keyword) => content.toLowerCase().includes(keyword.toLowerCase()));
    }
    async generateArticleFromThread(threadMessages) {
        try {
...
`;
    ...
    async generateSlideFromThread(threadMessages) {
        try {
...
---`,
    ...
    async generateTags(title, body) {
        try {
            ...
            if (!match)
                throw new Error('タグ抽出失敗: ' + content);
            const tagsRaw = JSON.parse(match[0]);
